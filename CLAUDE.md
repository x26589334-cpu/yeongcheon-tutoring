# 영천과외 (yeongcheon-tutoring) — 작업 규칙

경상북도 영천시 지역 타겟 과외 사이트. 정적 사이트, 빌드 없음.
...
## 도메인 / 배포
- 도메인: **nadostart.co.kr** (가비아, 2026-09-21 연결). `CNAME` 파일 있음 · 주소 `https://nadostart.co.kr/`
...
- 검색엔진 소유확인: 네이버 `6630a4fc387b1cb583c436db38f9154bf8d6f943` · 구글 `VaehpANuRkfJ6lrSCI5hs1588mTgqWFVy8IlgJD_MqQ` — 홈 `<head>` 에 생성기가 넣는다(`regions.js` 의 `verify`). **`index.html` 에 손으로 넣지 말 것** — 재생성하면 사라진다. 코드를 바꾸려면 `verify` 를 고치고 재생성.
...
| `schools.html` | 영천 초·중·고 39곳 목록 + 검색 |
| `school/*.html` | **학교별 정적 페이지 39개** — SEO 의 핵심. 학교당 국어·영어·수학·사회·과학·코딩 키워드 |
| `teachers.html` | 선생님 (화상 673명 — 영천 방문 선생님이 0명이라 방문 관련 내용은 감춰져 있다) |
...
5. `& "$env:USERPROFILE\사이트관리\로그기록.ps1" -Repo yeongcheon-tutoring -Status 완료 -What "글 N편"`
